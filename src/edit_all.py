...
def edit_all(current_dict):
	...
	while True:
		...
		#	cut_parens_access(current_dict['language'])
			...
			current_dict = fix_participles(current_dict,True)
		elif user_input == '6':
			current_dict = deduplicate(current_dict,True)
		elif user_input == '7':
			current_dict = fix_verbs(current_dict,True)
		elif user_input == '8':
			current_dict = remove_periods(current_dict)
		elif user_input == '9':
			current_dict = fix_sort_handles(current_dict)
		elif user_input == '10':
			current_dict = fix_etymology(current_dict,True)
		#elif user_input == '10':
...
def remove_spaces(current_dict):
	for i in range(len(current_dict['definitions'])):
		for j in range(len(current_dict['definitions'][i]['entries'])):
			for k in range(len(current_dict['definitions'][i]['entries'][j]['senses'])):
					text = current_dict['definitions'][i]['entries'][j]['senses'][k]
					text['gloss'] = text['gloss'].replace(" .",".").replace(" ,",",").replace(" :",":")
					current_dict['definitions'][i]['entries'][j]['senses'][k] = text		
	with open(current_dict['file'],mode = 'wb') as openFile:
		pickle.dump(current_dict, openFile)
	return current_dict	

def fix_etymology(current_dict,save_mode=False):
	for i in range(len(current_dict['definitions'])):
...
def replace_senses(current_dict):
	for i in range(len(current_dict['definitions'])):
		for j in range(len(current_dict['definitions'][i]['entries'])):
			for k in range(len(current_dict['definitions'][i]['entries'][j]['senses'])):
				current_dict['definitions'][i]['entries'][j]['senses'][k] = {'gloss':current_dict['definitions'][i]['entries'][j]['senses'][k],'tags':[]}
	openFile = open(current_dict['file'],mode = 'wb')
	pickle.dump(current_dict, openFile)
	openFile.close()	
	return current_dict
...
def remove_all_punct(current_dict):
	for i in range(len(current_dict['definitions'])):
		for j in range(len(current_dict['definitions'][i]['entries'])):
			for k in range(len(current_dict['definitions'][i]['entries'][j]['senses'])):
				if '\n' in current_dict['definitions'][i]['entries'][j]['senses'][k]:
					text = current_dict['definitions'][i]['entries'][j]['senses'][k]
					text = text[:text.find('\n')]
					current_dict['definitions'][i]['entries'][j]['senses'][k] = text		
	with open(current_dict['file'],mode = 'wb') as openFile:
...
def prune(entry):
	while True:
		...
		if user_input == 0:
			...
			entry['senses'] = entry['senses'][:user_input]
			...
			if user_input == '0':
				...
				while True:
					...
					if user_input == '0':
						...
						current_dict['definitions'][i]['entries'][0]['senses'] = edit_entry.move_entries(current_dict['definitions'][i]['entries'][0]['senses'],int(user_input[0]),int(user_input[1]))
				with open(current_dict['file'],mode = 'wb') as openFile:
					pickle.dump(current_dict, openFile)
			elif user_input == '6':
				...
				if selection != None:
					current_dict['definitions'][i]['entries'][0]['senses'][selection] = edit_entry.remove_words(current_dict['definitions'][i]['entries'][0]['senses'][selection])
					with open(current_dict['file'],mode = 'wb') as openFile:
						pickle.dump(current_dict, openFile)
	return current_dict
# END SPECIAL
...
def fix_participles(current_dict,save_mode=False):
	for i in range(len(current_dict['definitions'])):
		for j in range(len(current_dict['definitions'][i]['entries'])):
			if current_dict['definitions'][i]['entries'][j]['partOfSpeech'] == 'verb':
				...
				if current_dict['definitions'][i]['entries'][j]['partOfSpeech'] == 'participle':
					first = False
					for k in range(len(current_dict['definitions'][i]['entries'][j]['senses'])):
						text = current_dict['definitions'][i]['entries'][j]['senses'][k]['gloss']
						text, first = word_methods.participle_edit(text,first)
						current_dict['definitions'][i]['entries'][j]['senses'][k]['gloss'] = text
	if save_mode:
		openFile = open(current_dict['file'],mode = 'wb')
		pickle.dump(current_dict, openFile)
		openFile.close()	
...
def fix_verbs(current_dict,save_mode=False):
	for i in range(len(current_dict['definitions'])):
		for j in range(len(current_dict['definitions'][i]['entries'])):
			if current_dict['definitions'][i]['entries'][j]['partOfSpeech'] == 'verb':
				for k in range(len(current_dict['definitions'][i]['entries'][j]['senses'])):
...
def deduplicate(current_dict,save_mode=False):
...
def cut_parens(current_dict):
	for i in range(len(current_dict['definitions'])):
		for j in range(len(current_dict['definitions'][i]['entries'])):
			for k in range(len(current_dict['definitions'][i]['entries'][j]['senses'])):
				g = current_dict['definitions'][i]['entries'][j]['senses'][k]['gloss']
				if g[0] == "(":
					tags = ", ".join(current_dict['definitions'][i]['entries'][j]['senses'][k]['tags'])
					p = g[g.find("(") + 1:g.find(")")]
					if tags in p or tags.replace("-"," ") in p:
						current_dict['definitions'][i]['entries'][j]['senses'][k]['gloss'] = g[g.find(")") + 1:].strip(": ")
					else:
						for tag in current_dict['definitions'][i]['entries'][j]['senses'][k]['tags']:
...
def find_paren(current_dict):
	...
	for definition in current_dict['definitions']:
		for senses in definition['entries'][0]['senses']:
			if ')' in senses:
				text = senses[senses.find("("):senses.find(")")+1]
				print(text)
