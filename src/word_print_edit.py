		...
		# Option to delete
			...
			while not exit_inner_loop:
				...
				if s1 != None:
					...
					if s2 != None:
						new_word['entries'][s1]['senses'].extend(new_word['entries'][s2]['senses'])
...
def split_word(word,current_dict):
	...
	while True:
		...
		new_split['tags'] = []
		new_split['entries'] = [{'simpleParts':user_input,'principleParts':user_input,'partOfSpeech':'split','senses':[]}]
		for entry in word['entries']:
			while True:
				print(f"Split {count}:")
				if new_split['entries'][0]['senses']:
				...
				if type(selection) == list:
					for i in range(selection[0],selection[1]+1):
						new_split['entries'][0]['senses'].append(entry['senses'][i])
				else:
					new_split['entries'][0]['senses'].append(entry['senses'][selection])
		edit_entry.print_entry(new_split['entries'][0])
		while True:
