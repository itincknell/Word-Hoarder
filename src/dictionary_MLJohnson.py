...
def Johnson_OED(new_dictionary):
	...
	def pos_test(line,line_0 = ''):
		...
		if full_verb:
			...
			if see:
				...
				entry['senses'] = [{'gloss':'alternative form of ' + ' '.join(line[index+1:]).strip(',.;'),'tags':[]}]
	...
	def return_senses(line,index):
		...
		for i in range(len(sub_nums)):
			...
			if stop:
				senses.append({'gloss':' '.join(line[start:stop]).strip(',.;'),'tags':[]})
			else:
				senses.append({'gloss':' '.join(line[start:]).strip(',.;'),'tags':[]})
		return senses
	...
	def return_multi_entry(line):
		...
		for i in range(len(sub_nums)):
			...
			if entry == None or index == None:
				...
				entry['simpleParts'] = entry['principleParts'] = parts
				if stop:
					entry['senses'] = [{'gloss':' '.join(line[start:stop]).strip(',.;'),'tags':[]}]
				else:
					entry['senses'] = [{'gloss':' '.join(line[start:]).strip(',.;'),'tags':[]}]
			else:
				if debug_print:
					print("\t\tBBBBBBBB")
				if stop:
					if 'senses' not in entry:
						entry['senses'] = [{'gloss':' '.join(line[start+index:stop]).strip(',.;'),'tags':[]}]
				else:
					if 'senses' not in entry:
						entry['senses'] = [{'gloss':' '.join(line[start+index:]).strip(',.;'),'tags':[]}]
			...
			for line in f.readlines():
				...
				if line:
					...
					if index == None and not one_entry:
						...
						entry['senses'] = return_senses(line,index)
						new_word['entries'] = [entry]
						...
						if verb:
							...
							index = 1
							counter += 1		
						entry['senses'] = [{'gloss':' '.join(line[index:]).strip(',.;'),'tags':[]}]
						new_word['entries'] = [entry]	
					else:
						if 'senses' not in entry:
							entry['senses'] = [{'gloss':' '.join(line[index:]).strip(',.;'),'tags':[]}]
